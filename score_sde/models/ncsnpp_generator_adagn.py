...
class NCSNpp(nn.Module):
    ...
    def __init__(self, config):
        ...
        elif resblock_type == 'biggan_with_dit':
            ResnetBlock = functools.partial(ResnetBlockBigGAN_with_DiT,
                                            act=act,
                                            dropout=dropout,
                                            fir=fir,
                                            fir_kernel=fir_kernel,
        ...
        for i_level in range(num_resolutions):
            ...
            for i_block in range(num_res_blocks):
                ...
                if resblock_type == 'biggan_with_dit':
                    modules.append(ResnetBlock(in_ch=in_ch, out_ch=out_ch, size=config.current_resolution // (2 ** i_level)))
                else:
            ...
            if i_level != num_resolutions - 1:
                if resblock_type == 'ddpm':
                    ...
                    if resblock_type == 'biggan_with_dit':
                        modules.append(ResnetBlock(down=True, in_ch=in_ch, size=config.current_resolution // (2 ** i_level)))
                    else:
        ...
        in_ch = hs_c[-1]
        if resblock_type == 'biggan_with_dit':
            modules.append(ResnetBlock(in_ch=in_ch, size=config.current_resolution // (2 ** (num_resolutions - 1))))
        else:
        ...
        for i_level in reversed(range(num_resolutions)):
            for i_block in range(num_res_blocks + 1):
                ...
                if resblock_type == 'biggan_with_dit':
                    modules.append(ResnetBlock(in_ch=in_ch + hs_c.pop(),
                                           out_ch=out_ch, size=config.current_resolution // (2 ** i_level)))
                else:
                    modules.append(ResnetBlock(in_ch=in_ch + hs_c.pop(),
                                           out_ch=out_ch))
                in_ch = out_ch
            ...
            if i_level != 0:
                if resblock_type == 'ddpm':
                    ...
                    if resblock_type == 'biggan_with_dit':
                        modules.append(ResnetBlock(in_ch=in_ch, up=True, size=config.current_resolution // (2 ** i_level)))
                    else:
...
class Dit(nn.Module):
    ...
    def __init__(self, config):
        ...
        ResnetBlock = DiTBlock(hidden_size, num_heads=16, mlp_ratio=4.0)
