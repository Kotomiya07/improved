    ...
    for epoch in range(init_epoch, args.num_epoch + 1):
        ...
        for iteration, (x, y) in enumerate(data_loader):
            ...
            if args.sigmoid_learning and args.rec_loss:
                ...
                loss_g = loss_g + alpha[epoch]*rec_loss

            elif args.rec_loss and not args.sigmoid_learning:
                rec_loss = F.l1_loss(x_0_predict, real_data)
                loss_g = loss_g + rec_loss

            gmm = get_gradient_ratios(loss_g, loss_d, output)  # gamma

            grag_d_factor = 1.0 / (1.0 - gmm)
            
            loss_pack_fake = loss_fake - loss_g
            scaled_loss_pack_fake = loss_pack_fake * grag_d_factor
            ...
            if rank == 0:
                ...
                if args.sigmoid_learning:
                    print('\r[#{:05}][#{:04}][{:04.0f}ms] G Loss[{:.4f}] D Loss[{:.4f}] alpha[{:.4f}]'.format(
                        epoch, iteration, iter_time, loss_g.item(), loss_d.item(), alpha[epoch]), end="")
                elif args.rec_loss:
                    print('\r[#{:05}][#{:04}][{:04.0f}ms] G Loss[{:.4f}] D Loss[{:.4f}] rec_loss[{:.4f}]'.format(
                        epoch, iteration, iter_time, loss_g.item(), loss_d.item(), rec_loss.item()), end="")
                else:   
                    print('\r[#{:05}][#{:04}][{:04.0f}ms] G Loss[{:.4f}] D Loss[{:.4f}]'.format(
                        epoch, iteration, iter_time, loss_g.item(), loss_d.item()), end="")

                wandb.log({"G_loss_iter": loss_g.item(), "D_loss_iter": loss_d.item(), "iter_time": iter_time})
        ...
        if rank == 0:
            ...
            wandb.log({"G_loss": loss_g.item(), "D_loss": loss_d.item(), "alpha": alpha[epoch], "epoch_time": epoch_time / 1000})
            ########################################
