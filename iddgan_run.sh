...
if [[ $MODE == train ]]; then
	...
	elif [[ $DATASET == cifar10-diffit ]]; then
		python3 train_iddgan_diffit.py --dataset cifar10 --exp cifar10-diffit  --num_channels 4 --num_channels_dae 128 --num_timesteps 4 \
			--num_res_blocks 2 --batch_size 256 --num_epoch 2000 --ngf 64 --nz 50 --z_emb_dim 256 --n_mlp 4 --embedding_type positional \
			--use_ema --ema_decay 0.9999 --r1_gamma 0.02 --lr_d 1.25e-4 --lr_g 1.e-3 --lazy_reg 15 \
			--ch_mult 1 2 2 --save_content --datadir ./data/cifar-10 \
			--master_port $MASTER_PORT --num_process_per_node $GPUS --save_ckpt_every 5 \
			--current_resolution 16 --attn_resolutions 32 --num_disc_layers 3  --scale_factor 105.0 \
			--no_lr_decay \
	...
	elif [[ $DATASET == lsun-vqf4-sf6 ]]; then
		python3 train_iddgan_celeba.py --dataset lsun --image_size 256 --exp vq-f4-sf6 --num_channels 3 --num_channels_dae 128 --ch_mult 1 2 2 2 --num_timesteps 4 \
			--num_res_blocks 3 --batch_size 32 --num_epoch 1000 --ngf 64 --embedding_type positional --use_ema --ema_decay 0.999 --r1_gamma 2. \
			--nz 50 --z_emb_dim 256 --lr_d 1e-4 --lr_g 2e-4 --lazy_reg 10 --save_content --datadir data/lsun/ \
			--master_port $MASTER_PORT --num_process_per_node $GPUS \
			--current_resolution 64 --attn_resolution 16 --num_disc_layers 4  \
			--save_content_every 1 \
			--AutoEncoder_config ./autoencoder/config/vq-f4.yaml \
			--AutoEncoder_ckpt ./autoencoder/weight/vq-f4.ckpt \
			--scale_factor 6.0 \
