...
class GAN(nn.Module):
    def __init__(self, gen, dis, coeff, pos_coeff, args, device='cuda' if torch.cuda.is_available() else 'cpu'):
        super().__init__()
        self.device = device
        self.gen = gen
        self.dis = dis
...
def train(rank, gpu, args):
    ...
    model = GAN(netG, netD, coeff, pos_coeff, args, device)
        ...
        if rank == 0:
            ...
            if args.save_content:
                if epoch % args.save_content_every == 0:
                    print('\nSaving content.')
                    # # DDPモデルからジェネレータの状態を抽出
                    # if hasattr(model, 'module'):
                    #     # 分散学習の場合は.moduleからアクセス
                    #     generator_state = model.module.gen.state_dict()
                    #     discriminator_state = model.module.dis.state_dict()
                    # else:
                    #     # 単一GPUの場合は直接アクセス
                    #     generator_state = model.gen.state_dict()
                    #     discriminator_state = model.dis.state_dict()

                    # content = {'epoch': epoch + 1, 'global_step': global_step, 'args': args,
                    #            'netG_dict': generator_state, 'netD_dict': discriminator_state,
            ...
            if epoch % args.save_ckpt_every == 0:
                ...
                torch.save(generator_state, os.path.join(
